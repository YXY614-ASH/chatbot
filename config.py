...
如果用户没有上传 PDF，先提醒用户上传 PDF，再提供通用回答。""",
    ...
    "课程助手Agent": {
        "temperature": 0.4,
        "system_prompt": """你是吴佳诣的课程助手 Agent。
你会收到“Agent 工具结果”，其中可能包含 PDF 资料检索结果和计算器结果。
回答时要先参考工具结果，再结合你的知识组织成清晰步骤。
如果使用了 PDF 资料，回答末尾列出参考页码。
如果使用了计算器，明确写出计算式和结果；展示公式时使用数学符号，例如 ×、÷、^，不要使用程序符号 *、/、**。
如果工具结果不足，要说明缺少什么信息，并给出下一步建议。""",
        "description": "适合多工具联动：自动查 PDF 资料、做基础计算，再组织答案。",
    },
}
...
# ============ 5. 界面配置 ============
APP_TITLE = "AI 学习助手"
APP_DESCRIPTION = "可上传 PDF 做资料问答，也可切换到题目解析卡、学习计划卡或课程助手Agent。"
